...
if __name__ == "__main__":
    ...
    running_counts = lines.flatMap(lambda line: line.split(" "))\
                          .filter(lambda word: word == 'hi')      \
                          .map(lambda word: (word, 1))

    running_counts2 = lines.flatMap(lambda line: line.split(" "))\
                          .filter(lambda word: word == 'world')      \
                          .map(lambda word: (word, 1))

    union_rdd = running_counts.union(running_counts2).reduceByKey(lambda a, b: a + b)
    result = union_rdd.updateStateByKey(updateFunc)
    result.collect()
